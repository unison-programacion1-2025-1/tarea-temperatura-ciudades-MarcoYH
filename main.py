...
# Obtener las fechas y temperaturas - asegurando el formato correcto
fecha_hora_min = str(df_celsius.loc[temp_min_idx, 'Date Time']).strip()
temp_min = round(df_celsius.loc[temp_min_idx, 'Phoenix'], 2)

fecha_hora_max = str(df_celsius.loc[temp_max_idx, 'Date Time']).strip()
temp_max = round(df_celsius.loc[temp_max_idx, 'Phoenix'], 2)

# Calcular temperatura promedio
temp_promedio = round(df_celsius['Phoenix'].mean(), 2)

# Imprimir resultados con formato EXACTO
# Nota: Los tests parecen ser sensibles al formato exacto del texto
